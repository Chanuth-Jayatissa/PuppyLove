/*
  # Sample Data Population for PuppyLove App

  This migration populates the database with sample data for testing and development:

  1. Dog Avatars - Character types users can choose from
  2. Tags - Lifestyle and personality tags for user profiles  
  3. Shelters - Sample animal shelters with location data
  4. Dogs - Sample adoptable dogs with photos and details

  All inserts use existence checks to prevent duplicates and ensure idempotent execution.
*/

-- Insert Dog Avatars (only if they don't already exist)
DO $$
BEGIN
  -- Corgi
...
DECLARE
  ...
  tag_label text;
BEGIN
  FOREACH tag_label IN ARRAY tag_labels
  LOOP
    IF NOT EXISTS (SELECT 1 FROM tags WHERE label = tag_label) THEN
      INSERT INTO tags (label) VALUES (tag_label);
    END IF;
  END LOOP;
END $$;

-- Insert Shelters (only if they don't already exist)
DO $$
BEGIN
